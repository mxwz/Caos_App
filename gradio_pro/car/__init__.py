...
def get_external_ip():
    response = requests.get('https://cn.apihz.cn/api/ip/ipbaidu.php?id=设置id&key=设置key')
...
app4 = gr.Interface(
    fn=routing,
    inputs=[gr.Textbox(value="深圳信息职业技术大学", info="最好填写 省份＋城市＋区县＋城镇＋乡村＋街道＋门牌号码",
                       label="起始地点",
                       show_label=True, show_copy_button=True),
...
app5 = gr.Interface(
    fn=IP_address,
    inputs=gr.Textbox(value=ip1["ip"], info="填写外部IP地址，默认为本地外部IP", label="IP地址", show_label=True, show_copy_button=True),
